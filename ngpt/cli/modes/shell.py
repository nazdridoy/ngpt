...
from ngpt.ui.tui import spinner, copy_to_clipboard, get_terminal_input, get_multiline_input
...
def shell_mode(client, args, logger=None):
    ...
    # Normal shell mode functionality (direct stdin piping without --pipe flag)
        ...
        print("Enter shell command description (Ctrl+D or Ctrl+Z to submit):")
    ...
    if not prompt:
        print(f"{COLORS['yellow']}Error: Empty input. Please provide a command description.{COLORS['reset']}")
        return
